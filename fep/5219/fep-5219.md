...
Group actors SHOULD have `canFollow` and `canParticipate` properties:

- `canFollow`: this property specifies the affiliation required to follow the group (`member` for private groups, `none` for public and semi-private groups).
- `canParticipate`: this property specifies the affiliation required to participate in the group (`member` for private and semi-private groups, `none` for public groups).
...
The `canFollow` and `canParticipate` properties supersede the `manuallyApprovesFollowers` and `postingRestrictedToMods` properties.
